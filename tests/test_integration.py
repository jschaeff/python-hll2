...
def test_cumulative_union_sparse_full_representation():
    ...
    with open("tests/data/%s" % filename) as csv_file:
...
def float_cardinality(hll):
    ...
    if hll.get_type() == HLLType.EMPTY:
        return 0
    if hll.get_type() == HLLType.EXPLICIT:  # promotion has not yet occurred
        return hll.cardinality()
    if hll.get_type() == HLLType.SPARSE:
        return hll._sparse_probabilistic_algorithm_cardinality()
    if hll.get_type() == HLLType.FULL:
